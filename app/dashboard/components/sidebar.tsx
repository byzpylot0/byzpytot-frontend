...
import { ChevronLeft, ChevronRight, HomeIcon, Users2Icon, MessageCircle, Send, Calendar, Briefcase, Users } from "lucide-react";
    ...
    return (
        <div className={`h-full border-r flex flex-col overflow-y-auto bg-white shadow-lg transition-width duration-300 ${isCollapsed ? 'w-16' : 'w-64'}`}>
            ...
            <div className="flex flex-col w-full h-full p-4 space-y-2">
                ...
                {isRecruitmentOpen && (
                    <>
                        <NavItem href="/dashboard/recruitment/job-offers" icon={<Briefcase size={20} />} label="Job Offers" isCollapsed={isCollapsed} indent />
                        <NavItem href="/dashboard/recruitment/job-applications" icon={<Users size={20} />} label="Job Applications" isCollapsed={isCollapsed} indent />
                    </>
                )}
            </div>
